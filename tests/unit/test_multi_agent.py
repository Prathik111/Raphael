...
def world(tmp_path):
    ...
    registry.register(
        Tool(name="inspect", input_schema={"required": []}, risk_level=RiskLevel.LOW),
        _ok,
    )
    calls = {"danger": 0}
...
def test_10_concurrency_limits():
    ...
    for index in range(3):
        agents.register(
            AgentDefinition(id=f"a{index}", name=f"a{index}", allowed_tools=["slow"])
        )
    manager = AgentManager(agents, runtime, registry, max_workers=1)
    for index in range(3):
        manager.spawn(f"a{index}")
    records = [
        manager.submit(f"a{index}", "go slow", _plan("slow")) for index in range(3)
    ]
    try:
