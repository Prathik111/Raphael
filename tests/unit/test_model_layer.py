...
def test_malformed_structured_output_rejected():
    provider = MockModelProvider(
        "m", handler=lambda req: ModelResponse(text="not json{{")
    )
...
def _http_provider(monkeypatch, payload, status=200):
    ...
    provider = HttpChatModelProvider(
        endpoint="https://llm.test/v1", api_key="test-key-123",
        model="test-model")
...
def test_http_provider_posts_chat_completions(monkeypatch):
    ...
    provider, seen = _http_provider(monkeypatch, {
        "model": "test-model",
...
def test_http_provider_key_never_in_errors(monkeypatch):
    ...
    def boom(request, timeout=None):
        raise urllib.error.HTTPError(
            request.full_url, 401, "denied", {}, None)

    monkeypatch.setattr("urllib.request.urlopen", boom)
    provider = HttpChatModelProvider(
        endpoint="https://llm.test/v1", api_key="super-secret-key")
...
def test_http_provider_from_secrets_absent_returns_none():
    ...
    assert HttpChatModelProvider.from_secrets(
        DictSecretsProvider({})) is None
...
def test_normalize_endpoint_appends_chat_path():
    ...
    assert normalize_endpoint("https://h/v1/chat/completions") == \
...
def test_extract_json_block_strips_fences_and_prose():
    ...
    assert extract_json_block('Sure: {"a": {"b": "x} tricky"}} done') == \
        '{"a": {"b": "x} tricky"}}'
...
def test_request_structured_accepts_fenced_json():
    ...
    plan_json = ('{"goal": "g", "steps": [{"id": "s1", "description": "d", '
    ...
    assert request_structured(provider, ModelRequest(prompt="p"),
                              Plan).goal == "g"
...
def test_planner_repairs_rejected_draft():
    ...
    good = ('{"goal": "g", "steps": [{"id": "s1", "description": "d", '
...
def test_planner_raises_after_exhausted_repairs():
    ...
    backend = ModelReasoningBackend(
        MockModelProvider(
            "m", handler=lambda req: ModelResponse(text="not json at all")),
