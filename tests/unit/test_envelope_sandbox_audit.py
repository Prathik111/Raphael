...
from ai_ecosystem.security import AuditLog, AuthorizationManager, ComponentTrust, LocalSandboxProvider, SandboxProfile, TrustLevel, capability_risk, is_dangerous, meets
from ai_ecosystem.tools import GrantAllAuthorizer, ToolRegistry, ToolRunner


def _registry():
    registry = ToolRegistry()
    registry.register(Tool(name="ok", input_schema={"required": []}, risk_level=RiskLevel.LOW), lambda args: ToolResult(success=True, output="ok"))
    return registry
...
def test_no_self_elevation():
    ...
    with pytest.raises(DomainValidationError):
        trust.assign("plugin-x", TrustLevel.TRUSTED, actor="plugin-x", actor_trust=TrustLevel.LIMITED)
...
def test_sandbox_filesystem_restriction(tmp_path):
    ...
    def probe(args):
        import os as _os
        cwd = _os.getcwd()
        return ToolResult(success=True, output=f"cwd={cwd}\nexists={_os.path.isdir(cwd)}")
    tool = Tool(name="walk", input_schema={"required": []}, capabilities=["subprocess"])
    result = provider.run(tool, probe, {}, SandboxProfile(name="t", fs_root=str(tmp_path)), 5.0)
...
def test_sandbox_network_restriction():
    provider = LocalSandboxProvider()
    tool = Tool(name="fetch", input_schema={"required": []}, capabilities=["network"], network_access=True)
    with pytest.raises(Exception, match="network use denied"):
        provider.run(tool, lambda args: ToolResult(success=True), {}, SandboxProfile(name="t", allow_network=False), 5.0)
    result = provider.run(tool, lambda args: ToolResult(success=True, output="net"), {}, SandboxProfile(name="t", allow_network=True), 5.0)
    assert result.success
...
def test_sandbox_env_scrub():
    ...
    try:
        result = provider.run(Tool(name="probe", input_schema={"required": []}), probe, {}, SandboxProfile(name="t"), 5.0)
    finally:
...
def test_sandbox_cleanup_on_crash():
    ...
    with pytest.raises(ToolExecutionError, match="handler exploded"):
        provider.run(Tool(name="boom", input_schema={"required": []}), boom, {}, SandboxProfile(name="t"), 5.0)
    result = provider.run(Tool(name="ok", input_schema={"required": []}), lambda args: ToolResult(success=True, output="ok"), {}, SandboxProfile(name="t"), 5.0)
    assert result.success


def test_sandbox_requires_authorization_first(tmp_path):
    registry = ToolRegistry()
    registry.register(Tool(name="danger", input_schema={"required": []}, risk_level=RiskLevel.HIGH, requires_sandbox=True, sandbox_profile="strict"), lambda args: ToolResult(success=True, output="pwned"))
    from ai_ecosystem.security import Policy, PolicyEngine, RiskContext
    authorizer = AuthorizationManager(registry, policy_engine=PolicyEngine(Policy(name="default")), context=RiskContext(root=str(tmp_path)))
    runner = ToolRunner(registry, authorizer, sandbox=LocalSandboxProvider(), sandbox_profiles={"strict": SandboxProfile(name="strict")})
...
def test_sandbox_fail_closed_without_provider():
    registry = ToolRegistry()
    registry.register(Tool(name="danger", input_schema={"required": []}, risk_level=RiskLevel.LOW, requires_sandbox=True), lambda args: ToolResult(success=True, output="x"))
...
def test_sandboxed_execution_audited(tmp_path):
    ...
    try:
        log = AuditLog(db)
        registry = ToolRegistry()
        registry.register(Tool(name="ok", input_schema={"required": []}, risk_level=RiskLevel.LOW, requires_sandbox=True), lambda args: ToolResult(success=True, output="ok"))
        runner = ToolRunner(registry, GrantAllAuthorizer(), sandbox=LocalSandboxProvider(), sandbox_profiles={"default": SandboxProfile()}, auditor=log.as_recorder())
...
def test_audit_record_creation_and_query(tmp_path):
    ...
    try:
        log = AuditLog(db)
        log.record(action="tool.execute", actor="a1", actor_type="agent", task_id="t1", resource="filesystem.read", decision="GRANTED", risk="LOW", correlation_id="c1")
        log.record(action="tool.execute", actor="a1", task_id="t2", resource="terminal.execute", decision="DENIED", risk="HIGH", correlation_id="c2")
...
def test_runner_auditor_hook_covers_denials(tmp_path):
    ...
    try:
        log = AuditLog(db)
        registry = ToolRegistry()
        registry.register(Tool(name="danger", input_schema={"required": []}, risk_level=RiskLevel.HIGH), lambda args: ToolResult(success=True))
