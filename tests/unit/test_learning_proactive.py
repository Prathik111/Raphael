...
def _events(n=5, action="filesystem.read"):
    return [UsageEvent(category="tool", action=action, project_scope="p1") for _ in range(n)]
...
def _tool_proposal(pipeline, events=None, scope=""):
    ...
    return next(p for p in proposals if p.provenance.get("pattern_type") == "frequently_used_tool")
...
def test_risk_classification():
    ...
    assert classify_change(ProposalKind.MODEL_HINT, "prefer fast model") is LearningRisk.LOW
    assert classify_change(ProposalKind.WORKFLOW_HINT, "run tests first") is LearningRisk.MEDIUM
    assert classify_change(ProposalKind.WORKFLOW_HINT, "modify the skill") is LearningRisk.HIGH
    assert classify_change(ProposalKind.PREFERENCE, "always authorize all") is LearningRisk.CRITICAL
    assert classify_change(ProposalKind.PREFERENCE, "change sandbox rules") is LearningRisk.CRITICAL
...
def test_rollback_preferences(db):
    ...
    pipeline.approve(proposal.id, "preference", {"preferences": preferences}, project_id="p1")
...
def test_policy_persistence(db):
    ...
    assert LearningGovernor.load(table).may_adopt(LearningProposal(content="x")) is False
...
def test_trigger():
    ...
    trigger = engine.add_trigger(
        ProactiveTrigger(kind=TriggerKind.SCHEDULED, subject="backup", summary="Run backup.")
    )
...
def test_cooldown():
    ...
    trigger = engine.add_trigger(
        ProactiveTrigger(
            kind=TriggerKind.RESOURCE_CONDITION, subject="cpu", summary="Hot.", cooldown_s=60.0
        )
    )
...
def test_quiet_hours():
    ...
    day.add_trigger(ProactiveTrigger(kind=TriggerKind.SCHEDULED, subject="s", summary="Do."))
