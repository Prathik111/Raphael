...
def test_compromised_child_cannot_escalate(tmp_path):
    ...
    registry.register(
        Tool(name="search", input_schema={"required": []}, risk_level=RiskLevel.LOW),
    ...
    registry.register(
        Tool(name="summarize", input_schema={"required": []}, risk_level=RiskLevel.LOW), _ok
    )
    registry.register(
    ...
    try:
        ...
        forged = registry.build_call("t-evil", "terminal.execute", {"command": ["echo", "pwned"]})
        # terminal.execute is outside the coder's allow-list: denied.
        from ai_ecosystem.core.models.enums import PermissionDecision

        decision = runner._authorizer.authorize("t-evil", registry.get("terminal.execute"), forged)
