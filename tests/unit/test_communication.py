...
def world(tmp_path):
    ...
    registry.register(
        Tool(name="research", input_schema={"required": []}, risk_level=RiskLevel.LOW),
        _ok,
    )
    registry.register(
    ...
    for agent_id, role, tools in (
        ...
        agents.register(
            AgentDefinition(
                id=agent_id, name=agent_id, role=role, allowed_tools=list(tools)
            )
        )
...
def test_12_result_handoff(world):
    handoff = world["messages"].handoff(
        "r1", "c1", "t1", "facts: watts", correlation_id="corr-9"
    )
...
def test_18_ordering_where_required(world):
    ...
    assert [world["messages"].receive("c1").payload["n"] for _ in range(5)] == [
        0,
        1,
        2,
        3,
...
def test_21_supervisor_aggregation(world):
    ...
    outcome = coordinator.fan_out(
        ...
        [
            ...
            SubtaskSpec("c1", "build", _plan("code")),
        ],
        "corr-agg",
    )
...
def test_22_child_result_verification(world):
    ...
    outcome = coordinator.fan_out(
        "sup", [SubtaskSpec("r1", "find", _plan("research"))], "corr-ver"
    )
