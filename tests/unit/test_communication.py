...
def world(tmp_path):
    ...
    for agent_id, role, tools in (("r1", "research", ["research"]),
                                  ("c1", "coding", ["code"]),
                                  ("sup", "supervisor", [])):
        agents.register(AgentDefinition(id=agent_id, name=agent_id, role=role,
    ...
    yield {"db": db, "runtime": runtime, "bus": bus, "registry": registry,
           "agents": agents, "manager": manager, "messages": messages,
           "path": path}
    runtime.shutdown()
    db.close()


def _msg(sender="r1", recipient="c1", task_id="t1",
         message_type=MessageType.INFORMATION_RESPONSE, payload=None,
         correlation_id="corr-1"):
    return AgentMessage(sender=sender, recipient=recipient, task_id=task_id,
                        message_type=message_type, payload=payload or {"text": "hi"},
...
def test_10_task_request(world):
    world["messages"].send(_msg(message_type=MessageType.TASK_REQUEST,
                                payload={"goal": "do research"}))
    received = world["messages"].receive("c1")
...
def test_11_task_response(world):
    world["messages"].send(AgentMessage(
        sender="c1", recipient="sup", task_id="t1",
        message_type=MessageType.TASK_RESULT, payload={"success": True},
        correlation_id="corr-1"))
...
def test_12_result_handoff(world):
    handoff = world["messages"].handoff("r1", "c1", "t1", "facts: watts",
...
def test_13_permission_isolation(world):
    ...
    world["messages"].send(AgentMessage(
        sender="r1", recipient="c1", task_id="t1",
...
def test_15_loop_prevention(world):
    ...
    message = AgentMessage(sender="a", recipient="b", task_id="t",
                           message_type=MessageType.INFORMATION_RESPONSE,
                           payload={"n": 1}, correlation_id="loop")
    bus.send(message)
...
def test_16_bounded_agent_hops(world):
    ...
    bus.register("a")
    bus.register("b")
    message = bus.send(AgentMessage(
        sender="a", recipient="b", task_id="t",
        message_type=MessageType.INFORMATION_RESPONSE, payload={},
        correlation_id="c"))
...
def test_17_concurrent_messages(world):
    ...
    def spray(index):
        try:
            for n in range(20):
                world["messages"].send(AgentMessage(
                    sender="r1", recipient="c1", task_id=f"t{index}-{n}",
                    message_type=MessageType.TASK_PROGRESS,
                    payload={"n": n}, correlation_id=f"c{index}"))
...
def test_18_ordering_where_required(world):
    for n in range(5):
        world["messages"].send(AgentMessage(
            sender="r1", recipient="c1", task_id="t",
            message_type=MessageType.TASK_PROGRESS, payload={"n": n},
...
def test_21_supervisor_aggregation(world):
    coordinator = Coordinator(world["manager"], world["messages"],
                              verifier=Verifier(), bus=world["bus"])
    outcome = coordinator.fan_out("sup", [
        SubtaskSpec("r1", "find", _plan("research")),
        SubtaskSpec("c1", "build", _plan("code"))], "corr-agg")
...
def test_22_child_result_verification(world):
    coordinator = Coordinator(world["manager"], world["messages"],
                              verifier=Verifier())
    outcome = coordinator.fan_out("sup", [
        SubtaskSpec("r1", "find", _plan("research"))], "corr-ver")
...
def test_message_delivery_perf_smoke(world):
    ...
    for n in range(1000):
        bus.send(AgentMessage(
            sender="r1", recipient="c1", task_id=f"t{n}",
            message_type=MessageType.TASK_PROGRESS, payload={"n": n},
            correlation_id="perf"))
