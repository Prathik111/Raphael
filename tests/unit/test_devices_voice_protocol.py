...
def test_phone_replay_protection(wired):
    gateway, _, _, _ = wired
    _phone, token = _paired_phone(wired)
...
def test_voice_transcription(wired):
    ...
    session = VoiceSession(
        phone, token, MockSTT({"a1": "status of my tasks"}), MockTTS()
    )
...
def test_voice_cancellation(wired):
    ...
    session = VoiceSession(
        phone, token, MockSTT({"a1": "cancel the monthly report"}), MockTTS()
    )
    out = session.handle("a1")
...
def test_voice_disabled(wired):
    phone, token = _paired_phone(wired)
    session = VoiceSession(
        phone, token, MockSTT({"a1": "hi"}), MockTTS(), enabled=False
    )
...
def test_protocol_authorization_context():
    validator = _allow_all()
    envelope = envelope_for(
        "pc", "pc", "X", auth=AuthRef(scheme="hmac", key_id="esp32-1")
    )
