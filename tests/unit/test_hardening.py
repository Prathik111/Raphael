...
def test_circuit_closed_to_open_to_half_open():
    ...
    breaker = CircuitBreaker(
        failure_threshold=2, reset_timeout_s=10.0, clock=lambda: clock[0]
    )
...
def test_startup_recovery(tmp_path):
    ...
    record = manager.submit(
        "a1", "half", Plan(goal="g", steps=[], final_verification="v")
    )
    ...
    job = scheduler_repo.create(
        ScheduledJob(goal="j", requirements=ComputeRequirements())
    )
    ...
    for path in (
        ...
        for lineno, line in enumerate(
            ...
            if match and not any(
                marker in match.group(2).lower() for marker in allowed_markers
            ):
...
def test_resource_safety_guards():
    ...
    assert (
        RetryPolicy(max_attempts=3).should_retry(
            Classification(FailureClass.TIMEOUT, "t", "s"), 99
        )
        is False
    )
