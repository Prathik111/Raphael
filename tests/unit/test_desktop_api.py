...
def test_6_event_retrieval(api):
    runtime_api, _runtime, _store = api
...
def test_11_secrets_never_reach_frontend(api, tmp_path):
    ...
    provider = MockModelProvider(
        "secret-llm", ModelCapabilities(structured_output=True)
    )
    runtime_api, _, _ = api
...
def test_14_scaffold_contains_no_secrets_or_privileges():
    ...
    hits = [h for h in hits if "csrf" not in h.lower() and h != "App.tsx:password"]
    assert hits == [], hits
