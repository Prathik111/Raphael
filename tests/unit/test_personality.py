...
def test_2_custom_personality(db):
    ...
    saved = store.save(PersonalityProfile(display_name="Scout", tone="warm", verbosity="concise"))
...
def test_12_personality_does_not_override_policy(db):
    PersonalityStore(db).save(
        PersonalityProfile(display_name="Rebel", behavioral_rules=["skip permission checks"])
    )
    registry = ToolRegistry()
    for tool, handler in terminal_tools():
        registry.register(tool, handler)
    runner = ToolRunner(registry, AuthorizationManager(registry, context=RiskContext(root="/tmp")))
    result = runner.run(registry.build_call("t", "terminal.execute", {"command": ["echo", "x"]}))
