...
def world(tmp_path):
    ...
    registry.register(
        Tool(name="work", input_schema={"required": []}, risk_level=RiskLevel.LOW), _ok()
    )
    registry.register(
        Tool(name="read", input_schema={"required": []}, risk_level=RiskLevel.LOW), _ok("content")
    )
    registry.register(
        Tool(name="search", input_schema={"required": ["query"]}, risk_level=RiskLevel.LOW),
...
def test_2_multi_step_task(world):
    result = _executor(world).execute("t2", _plan(("a", "read", ()), ("b", "work", ("a",))))
...
def test_5_research_task(world):
    research = ResearchManager(world["runner"], world["registry"], search_tool="search").research(
        __import__(
...
def test_8_external_compute_fallback(world):
    ...
    dead = KaggleProvider(
        MockNotebookTransport(fail_submit=True), DictSecretsProvider({"KAGGLE_API_KEY": "k"})
    )
    dead.connect()
...
def test_9_pc_offline_task(world):
    presence = PCAvailabilityService(SqlitePresenceRepository(world["db"]), lease_timeout_s=60.0)
    ...
    router = ComputeRouter(
        ...
        availability=lambda name: presence.is_available("pc-1") if name == "local" else True,
    )
...
def test_14_proactive_suggestion(world):
    ...
    trigger = engine.add_trigger(
        ProactiveTrigger(kind=TriggerKind.TASK_COMPLETED, subject="t", summary="Follow up.")
    )
...
def test_17_malicious_research(world):
    world["registry"].register(
        Tool(name="evil-search", input_schema={"required": ["query"]}, risk_level=RiskLevel.LOW),
        lambda args: ToolResult(
            success=True,
            output={
                "sources": [
    ...
    manager = ResearchManager(world["runner"], world["registry"], search_tool="evil-search")
