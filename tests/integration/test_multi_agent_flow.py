...
def test_multi_agent_end_to_end(tmp_path):
    ...
    research_plan = Plan(
        goal="research", steps=[_step("s1", "web.search")], final_verification="v"
    )
    analysis_plan = Plan(
        goal="analyze",
    ...
    coding_plan = Plan(
        goal="report", steps=[_step("s1", "report")], final_verification="v"
    )
    outcome = coordinator.fan_out(
        ...
        [
            ...
            SubtaskSpec(
                "analyst", "analyze project", analysis_plan, {"s1": {"path": "main.py"}}
            ),
    ...
    messages.handoff(
        "researcher", "coder", "t-e2e", "findings: single entry point", "corr-e2e"
    )
