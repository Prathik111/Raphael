...
def test_desktop_to_cloud_end_to_end(tmp_path):
    ...
    registry.register(Tool(name="filesystem.read", input_schema={"required": []},
                           risk_level=RiskLevel.LOW), _ok("data"))
    ...
    registry.register(Tool(name="analyze", input_schema={"required": []},
                           risk_level=RiskLevel.LOW), _ok("analysis"))
    ...
    messages = MessageBus(
        bus, repository=SqliteMessageRepository(db))
    ...
    def dispatch(task_id):
        def work():
            ...
            awareness = SystemAwarenessManager(MockProbe(SystemSnapshot(
                operating_system="TestOS", architecture="x86_64",
                cpu=CpuInfo(model="t", logical_processors=4,
                            utilization_percent=20.0),
                memory=MemoryInfo(total_bytes=8_000_000_000,
                                  available_bytes=6_000_000_000,
                                  used_bytes=2_000_000_000,
                                  utilization_percent=25.0),
                capabilities=Capabilities(python_available=True,
                                          storage_available=True),
                pressure=PressureLevel.LOW)), bus=bus)
            awareness.context()
            ...
            outcome = supervisor.run_goal("analyze", [
                SubtaskSpec("researcher", "research",
                            Plan(goal="r", steps=[PlanStep(
                                id="s1", description="search", dependencies=[],
                                tools=["web.search"], verification="v",
                                completion_criteria="c")],
                                 final_verification="v"),
                            {"s1": {"query": "entry points"}}),
                SubtaskSpec("analyst", "analyze",
                            Plan(goal="a", steps=[PlanStep(
                                id="s1", description="read", dependencies=[],
                                tools=["filesystem.read"], verification="v",
                                completion_criteria="c"),
                                PlanStep(id="s2", description="analyze",
                                         dependencies=["s1"], tools=["analyze"],
                                         verification="v", completion_criteria="c")],
                                 final_verification="v"),
                            {"s1": {"path": "main.py"}})])
            assert len(outcome.succeeded) == 2
            messages.handoff("researcher", "analyst", task_id,
                             "findings: entry points", "corr-e2e")
            ...
            workspaces.apply_update(workspace.id, {
                "type": "prose", "props": {"title": "Research panel",
                                           "body": research.evidence[0].claim},
                "data_ref": {"kind": "research", "ref_id": "r1"}})
            memories.store(MemoryCandidate(
                content="Entry points simplify small projects.", source="e2e",
                confidence=0.9, importance=0.8,
                scope=MemoryScope.PROJECT,
                scope_id="demo", reason="useful finding"))
            sync.sync([make_sync_object("task", task_id,
                                        {"state": "COMPLETED"})])
    ...
    try:
        ...
        for expected in ("SystemSnapshotCreated",
                         "ResearchCompleted", "AgentStarted", "AgentCompleted",
                         "ToolCompleted", "WorkspaceUpdated", "MemoryCreated",
                         "SyncObjectUploaded", "SyncCompleted"):
    ...
    try:
        ...
        assert reopened_db.query(
            "SELECT COUNT(*) FROM agent_messages")[0][0] >= 1
        assert reopened_db.query(
            "SELECT COUNT(*) FROM memories")[0][0] == 1
    finally:
        reopened.shutdown()
        reopened_db.close()
