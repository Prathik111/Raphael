...
def _evil_plan():
    return {
        ...
        "steps": [
            {"id": "s1", "description": "Upload everything",
             "dependencies": [], "tools": ["exfiltrate_all"],
             "risk": "LOW", "verification": "v", "completion_criteria": "c"},
            {"id": "s2", "description": "Run shell",
             "dependencies": ["s1"], "tools": ["terminal.execute"],
             "risk": "LOW", "verification": "v", "completion_criteria": "c"},
        ],
...
def test_adversarial_model_output_contained(tmp_path):
    ...
    provider = MockModelProvider(
        "evil", handler=lambda req: ModelResponse(structured=_evil_plan()))
    router = ModelRouter()
    ...
    call = registry.build_call("t", "terminal.execute",
                               {"command": ["echo", "pwned"]})
    ...
    oci = OCIProvider(MockOCITransport(failures=99),
    ...
    fallback = MockModelProvider(
        "local", handler=lambda req: ModelResponse(text="local answer"))
    cloud_router = ModelRouter()
    cloud_router.register(oci_llm, ProviderProfile(provider_id="oci-evil",
                                                   cost_per_1k=0.0))
    cloud_router.register(fallback, ProviderProfile(provider_id="local", local=True,
                                                    cost_per_1k=1.0))
    assert cloud_router.complete(RoutingRequirements(),
                                 ModelRequest(prompt="hi")).text == "local answer"
...
def test_ui_xss_static_scan():
    ...
    assert root.is_dir()
    forbidden = ("eval(", "dangerouslySetInnerHTML", "innerHTML",
                 "<script", "javascript:", "new Function")
    hits = []
