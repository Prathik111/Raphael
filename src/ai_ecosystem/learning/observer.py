...
class UsageObserver:
    ...
    def observe_tool(
        self, tool: str, success: bool, duration_ms: float = 0.0, project: str = "", agent: str = ""
    ) -> UsageEvent | None:
...
class UsageAggregator:
    ...
    def success_rate(self, category: str, action: str) -> float | None:
        ...
        relevant = [e for e in self._events if e.category == category and e.action == action]
        if not relevant:
            return None
...
class ProposalEngine:
    ...
    def propose(self, pattern: UsagePattern) -> LearningProposal:
        ...
        kind = self._KIND_FOR_PATTERN.get(pattern.pattern_type, ProposalKind.WORKFLOW_HINT)
    ...
    def adopt_to_memory(self, proposal: LearningProposal, memories: Any) -> Any:
        ...
        self._emit(
            EventType.PROPOSAL_ADOPTED, "", {"proposal_id": proposal.id, "memory_id": created.id}
        )
        return created
    ...
    def adopt_to_preferences(
        ...
        if proposal.kind not in (
            ...
            raise DomainValidationError(f"kind {proposal.kind.value} cannot become a preference")
    ...
    def reject(self, proposal: LearningProposal, reason: str = "") -> LearningProposal:
        ...
        self._emit(EventType.PROPOSAL_REJECTED, "", {"proposal_id": proposal.id, "reason": reason})
        return proposal
