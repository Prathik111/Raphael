...
_FORBIDDEN_KINDS = ("policy", "policies", "permission", "permissions",
                    "authorize", "authorized", "authorization", "risk",
                    "sandbox", "secret", "secrets")
...
class UsageObserver:
    ...
    def observe_model(self, model: str, success: bool, task_kind: str = "",
        ...
        return self.record(UsageEvent(
            ...
            duration_ms=0.0, success=success,
    ...
    def record(self, event: UsageEvent) -> UsageEvent | None:
        ...
        if mode is ObservationMode.DISABLED:
            return None
        if mode in (ObservationMode.LOCAL_PERSISTENCE, ObservationMode.LEARNING_ENABLED):
            if self._repository is None:
                raise DomainValidationError(
                    "persistence mode requires a repository")
        ...
        for (category, action), count in sorted(aggregator.counts().items()):
            if category == "tool" and count >= self._min_evidence:
                patterns.append(UsagePattern(
                    ...
                    evidence_count=count, confidence=self._confidence(count),
                    scope=scope, evidence=[f"{category}:{action}x{count}"]))
        ...
        for sequence, count in sorted(aggregator.sequences("tool").items()):
            if count >= self._min_evidence:
                patterns.append(UsagePattern(
                    ...
                    evidence_count=count, confidence=self._confidence(count),
                    scope=scope, evidence=[" -> ".join(sequence)] * count))
        return patterns
...
class ProposalEngine:
    ...
    def propose(self, pattern: UsagePattern) -> LearningProposal:
        ...
        kind = self._KIND_FOR_PATTERN.get(pattern.pattern_type,
                                          ProposalKind.WORKFLOW_HINT)
        content = self._content_for(pattern, kind)
        if _has_forbidden_word(content, _FORBIDDEN_KINDS):
            raise DomainValidationError(
                "proposals must never touch security policy")
        proposal = LearningProposal(
            kind=kind, content=content, pattern_id=pattern.id,
            confidence=pattern.confidence, scope=pattern.scope,
            provenance={"pattern_type": pattern.pattern_type,
                        "evidence": list(pattern.evidence),
                        "evidence_count": pattern.evidence_count},
        )
        self._emit(EventType.LEARNING_PROPOSAL_CREATED, "",
    ...
    def adopt_to_memory(self, proposal: LearningProposal,
                        memories: Any) -> Any:
        ...
        candidate = MemoryCandidate(
            content=proposal.content, type=MemoryType.EXPERIENCE,
            source=f"learning:{proposal.pattern_id}",
            confidence=proposal.confidence, importance=proposal.confidence,
            scope=MemoryScope.GLOBAL, reason="adopted learning proposal",
            metadata={"proposal_id": proposal.id},
        )
        created = memories.store(candidate)
        proposal.status = ProposalStatus.ADOPTED
        self._emit(EventType.PROPOSAL_ADOPTED, "",
                   {"proposal_id": proposal.id, "memory_id": created.id})
        return created

    def adopt_to_preferences(self, proposal: LearningProposal,
                             preferences: Any, project_id: str = "") -> Any:
        ...
        self._require_proposed(proposal)
        if proposal.kind not in (ProposalKind.WORKFLOW_HINT, ProposalKind.MODEL_HINT,
    ...
    def reject(self, proposal: LearningProposal, reason: str = "") -> LearningProposal:
        ...
        self._emit(EventType.PROPOSAL_REJECTED, "",
    ...
    def _require_proposed(proposal: LearningProposal) -> None:
        if proposal.status is not ProposalStatus.PROPOSED:
            raise DomainValidationError(
                f"proposal already {proposal.status.value}")
