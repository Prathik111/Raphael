...
class SkillRegistry:
    ...
    def __init__(
        self, repository: SqliteSkillRepository, bus: EventBus | None = None
    ) -> None:
    ...
    def lookup(self, name: str, version: str | None = None) -> Skill | None:
        ...
        if not active:
            return None
        return max(active, key=lambda s: _version_key(s.version))
    ...
    def announce_outcome(
        self, skill: Skill, task_id: str, success: bool, detail: str = ""
    ) -> None:
...
class SkillPlanBuilder:
    ...
    def build(
        ...
        for index, template in enumerate(skill.workflow):
            ...
            step_arguments = {
                key: (
                    self._fill(str(value), inputs) if isinstance(value, str) else value
                )
            ...
            steps.append(
                PlanStep(
                    id=step_id,
                    description=self._fill(
                        str(template.get("description", tool_name)), inputs
                    ),
    ...
    def check_policy(self, skill: Skill, agent_id: str = "") -> tuple[bool, str]:
        ...
        for tool_name in skill.allowed_tools:
            ...
            assessment = self._risk.assess(
                "",
                tool,
                ToolCall(task_id="", tool=tool_name),
...
class SkillCandidateStore:
    ...
    def approve(
        self, proposal_id: str, registry: SkillRegistry, version: str = "1.0.0"
    ) -> Skill:
