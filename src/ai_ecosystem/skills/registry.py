...
class SkillRegistry:
    ...
    def __init__(self, repository: SqliteSkillRepository, bus: EventBus | None = None) -> None:
    ...
    def register(self, skill: Skill) -> Skill:
        ...
        if not _valid_version(skill.version):
            raise DomainValidationError(f"invalid skill version {skill.version!r}")
        ...
        self._emit(
            ...
            "",
            {"skill_id": created.id, "name": created.name, "version": created.version},
        )
        return created
    ...
    def new_version(self, name: str, **changes: Any) -> Skill:
        ...
        self._emit(
            ...
            "",
            {"skill_id": created.id, "name": name, "version": created.version},
        )
        return created
    ...
    def select(
        ...
        self._emit(
            EventType.SKILL_SELECTED, "", {"query": query, "selected": [s.name for s in found]}
        )
        return found

    def announce_outcome(self, skill: Skill, task_id: str, success: bool, detail: str = "") -> None:
        ...
        self._emit(
            ...
            task_id,
            {"skill_id": skill.id, "name": skill.name, "version": skill.version, "detail": detail},
        )
    ...
    def build(
        ...
        for index, template in enumerate(skill.workflow):
            ...
            if not isinstance(raw_arguments, dict):
                raise DomainValidationError(f"step {step_id!r} arguments must be a mapping")
            step_arguments = {
                key: (self._fill(str(value), inputs) if isinstance(value, str) else value)
        ...
        plan = Plan(
            ...
            final_verification=str(
                skill.verification[0].get("check", "") if skill.verification else ""
            ),
        )
        ...
        if self._bus is not None:
            self._bus.publish(
                Event(
                    ...
                    task_id="",
                    payload={"skill_id": skill.id, "name": skill.name, "version": skill.version},
                )
            )
    ...
    def check_policy(self, skill: Skill, agent_id: str = "") -> tuple[bool, str]:
        ...
        for tool_name in skill.allowed_tools:
            ...
            assessment = self._risk.assess(
                "", tool, ToolCall(task_id="", tool=tool_name), RiskContext(agent_id=agent_id)
            )
...
class SkillCandidateStore:
    ...
    def propose(self, proposal: SkillProposal) -> SkillProposal:
        ...
        if self._bus is not None:
            self._bus.publish(
                Event(
                    event_type=EventType.SKILL_PROPOSAL_CREATED,
                    task_id="",
    ...
    def approve(self, proposal_id: str, registry: SkillRegistry, version: str = "1.0.0") -> Skill:
...
def propose_from_workflow(
    plan: Plan, name: str, author: str = "", scope_id: str = ""
) -> SkillProposal:
    ...
    return SkillProposal(
        name=name,
        description=f"Learned from plan: {plan.goal}",
