...
def approval_hash(tool_name: str, arguments: dict[str, Any], policy_name: str) -> str:
    canonical = json.dumps({"tool": tool_name, "arguments": arguments, "policy": policy_name},
...
class ApprovalStore:
    ...
    def request(self, task_id: str, tool_name: str, arguments: dict, risk: str,
                reason: str, policy_name: str, ttl_s: float | None = None) -> ApprovalRequest:
        ...
        with self._lock:
            ...
            if ttl <= 0:
                from ai_ecosystem.core.errors.exceptions import DomainValidationError
                raise DomainValidationError("approval TTL must be positive")
            request = ApprovalRequest(task_id=task_id, tool=tool_name, arguments=dict(arguments),
    ...
    def pending(self, task_id: str = "") -> list[ApprovalRequest]:
        with self._lock:
            self.sweep_expired()
            all_requests = self._repo.list() if self._repo is not None else list(self._memory.values())
            return [r for r in all_requests if r.status is ApprovalStatus.PENDING
    ...
    def decide(self, approval_id: str, approved: bool,
        ...
        with self._lock:
            ...
            if request.status is not ApprovalStatus.PENDING:
                raise DomainValidationError(f"approval {approval_id!r} is already {request.status.value}")
    ...
    def _expire_one(self, request: ApprovalRequest) -> None:
        if (request.status is ApprovalStatus.PENDING and request.expires_at is not None
