...
class DataPolicy(BaseModel):
    ...
    def decide(self, value: object, destination: str, *, declared: DataClass | None = None) -> EgressDecision:
        ...
        if classification is DataClass.SECRET:
            return EgressDecision(False, classification, destination, "SECRET data may never leave the local trust boundary")
        if classification is DataClass.SENSITIVE:
            allowed = destination in self.approved_sensitive_destinations
            return EgressDecision(allowed, classification, destination,
                                   "approved sensitive destination" if allowed else "SENSITIVE destination not approved")
        ...
        return EgressDecision(destination in self.approved_sensitive_destinations or destination in self.approved_personal_destinations,
                               classification, destination, "destination is not explicitly approved")


def _contains_secret(value: object) -> bool:
    if isinstance(value, dict):
        return any(looks_secret(str(k)) or _contains_secret(v) or looks_like_secret_value(v)
...
def _looks_personal(value: object) -> bool:
    if isinstance(value, dict):
        ...
        return bool(keys & {"email", "phone", "address", "name", "location", "dob", "date_of_birth"})
    return False


def require_egress(policy: DataPolicy, value: object, destination: str,
                   *, declared: DataClass | None = None) -> None:
