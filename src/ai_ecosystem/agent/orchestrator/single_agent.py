...
class SingleAgent:
    ...
    def run_goal(self, goal_text: str, config: AgentConfig | None = None,
        ...
        return self._drive(task, ctx, goal_text, clock, total_started,
                           cancel_token)
    ...
    def _drive(self, task: Task, ctx: ExecutionContext, goal_text: str,
        ...
        try:
            ...
            return self._complete(task, ctx, plan, execution, verifications,
    ...
    def resume(self, task_id: str, config: AgentConfig | None = None) -> AgentResult:
        ...
        try:
            ...
            recovery = self._recover_if_needed(
                task, ctx, ctx.plan, execution, verifications, clock)
            if recovery is not None:
                verifications = self._reverify(recovery)
            memory_ids = self._memorize(task, ctx, clock)
    ...
    def _understand(self, task: Task, ctx: ExecutionContext,
                    goal_text: str, clock: _Clock) -> TaskSpec:
        ...
        try:
            spec = request_structured(
                provider,
                ModelRequest(prompt=f"UNDERSTAND: {goal_text}",
        ...
        self._emit(EventType.UNDERSTANDING_COMPLETED, task.id,
    ...
    def _aware(self, task: Task, ctx: ExecutionContext, clock: _Clock) -> None:
        ...
        if workspace and self._registry.has("filesystem.list"):
            call = self._registry.build_call(
                task.id, "filesystem.list", {"path": workspace})
    ...
    def _research_phase(self, task: Task, ctx: ExecutionContext,
                        goal_text: str, clock: _Clock) -> None:
        started = time.monotonic()
    ...
    def _plan(self, task: Task, ctx: ExecutionContext,
        ...
        self._emit(EventType.PLAN_CREATED, task.id,
                   {"steps": len(plan.steps), "goal": plan.goal})
    ...
    def _execute(self, task: Task, ctx: ExecutionContext, plan: Plan,
        ...
        execution = self._executor().execute_graph(
            task.id, TaskGraph.from_plan(plan),
            arguments=self._config.arguments, context=ctx,
    ...
    def _verify(self, task: Task, ctx: ExecutionContext, plan: Plan,
                execution: ExecutionResult, clock: _Clock) -> dict[str, VerificationResult]:
        started = time.monotonic()
        self._walk_to(task, TaskState.VERIFYING)
        out: dict[str, VerificationResult] = {}
        for step in plan.steps:
            out[step.id] = self._verify_one(
                task.id, step.id, execution.results.get(step.id, []))
    ...
    def _recover_if_needed(self, task: Task, ctx: ExecutionContext, plan: Plan,
        ...
        try:
            if execution.status is OverallStatus.COMPLETED and all(
                    v.status is VerificationStatus.PASSED
            ...
            planner = (self._recovery_planner_factory()
                       if self._recovery_planner_factory else None)
            ...
            def verify_fn(task_id: str, current: Plan,
                          result: ExecutionResult) -> VerificationResult:
                merged = [self._verify_step(task_id, step.id,
                                            result.results.get(step.id, []))
                          for step in current.steps]
                return self._worst(task_id, merged)

            outcome = engine.run_with_prior(
                task.id, plan, execute_fn, verify_fn,
                self._config.arguments, self._config.max_attempts,
                prior_result=execution,
                prior_verification=self._worst(
                    task.id, list(verifications.values())),
            )
    ...
    def _memorize(self, task: Task, ctx: ExecutionContext,
                  clock: _Clock) -> list[str]:
        started = time.monotonic()
        try:
            if self._memories is None:
                return []
    ...
    def _complete(self, task: Task, ctx: ExecutionContext, plan: Plan,
        ...
        if current is not None and current.state is TaskState.CANCELLED:
            return self._cancelled(task, ctx, "cancelled by operator",
                                   clock, total_started)
        ...
        result = AgentResult(
            ...
            verification_status=worst.status.value, recovery=recovery,
            memory_ids=memory_ids,
            evidence=[o for o in ctx.observations if o.startswith("evidence ")],
            timings=dict(clock.marks), error="" if status == "COMPLETED" else worst.reason,
        )
        self._store_result(ctx, result)
        self._runtime.contexts_repo.save(ctx)
        return result

    def _cancelled(self, task: Task, ctx: ExecutionContext, error: str,
                   clock: _Clock, total_started: float) -> AgentResult:
        ...
        try:
            ...
            clock.marks["total_s"] = round(time.monotonic() - total_started, 3)
            result = AgentResult(task_id=task.id, status="FAILED",
                                 summary="Cancelled by the operator before "
                                         "completion.",
                                 timings=dict(clock.marks), error=error)
            self._store_result(ctx, result)
            self._runtime.contexts_repo.save(ctx)
        return result

    def _fail(self, task: Task, ctx: ExecutionContext, error: str,
              clock: _Clock, total_started: float) -> AgentResult:
        try:
            current = self._runtime.manager.get_task(task.id)
            if current is not None and current.state not in (
            ...
            clock.marks["total_s"] = round(time.monotonic() - total_started, 3)
            result = AgentResult(task_id=task.id, status="FAILED", summary="",
                                 timings=dict(clock.marks), error=error)
            self._store_result(ctx, result)
            self._runtime.contexts_repo.save(ctx)
        return result
    ...
    def _verify_one(self, task_id: str, step_id: str,
                    results: list) -> VerificationResult:
        criteria = self._config.verification_criteria.get(
            step_id, [{"strategy": "command_results"}])
        return self._verifier.verify(
            task_id, step_id, results, criteria,
            root=self._config.verification_root)

    def _verify_step(self, task_id: str, step_id: str,
                     results: list) -> VerificationResult:
        return self._verify_one(task_id, step_id, results)

    @staticmethod
    def _worst(task_id: str, results: list[VerificationResult]) -> VerificationResult:
        order = {VerificationStatus.FAILED: 0, VerificationStatus.ERROR: 1,
                 VerificationStatus.INCONCLUSIVE: 2, VerificationStatus.PASSED: 3,
                 VerificationStatus.PENDING: 4}
        if not results:
            return VerificationResult(task_id=task_id, status=VerificationStatus.INCONCLUSIVE,
    ...
    def _summarize(self, ctx: ExecutionContext, execution: ExecutionResult,
        ...
        if self._personalization is not None:
            context = self._personalization.build_context(
                task_id=task.id, project_id=self._config.project_id,
                agent_id=self._config.agent_id, query=ctx.goal)
    ...
    def _walk_to(self, task: Task, target: TaskState) -> None:
        ...
        _PATHS: dict[tuple[TaskState, TaskState], list[TaskState]] = {
            (TaskState.VERIFYING, TaskState.EXECUTING):
            ...
            (TaskState.RECOVERING, TaskState.VERIFYING):
                [TaskState.EXECUTING, TaskState.VERIFYING],
        }
        _FORWARD = [
            TaskState.CREATED, TaskState.UNDERSTANDING, TaskState.AWARENESS,
            TaskState.PLANNING, TaskState.WAITING_PERMISSION,
            TaskState.EXECUTING, TaskState.VERIFYING,
        ]
        current = self._runtime.manager.get_task(task.id).state
