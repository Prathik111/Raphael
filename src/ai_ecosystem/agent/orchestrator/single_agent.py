...
class SingleAgent:
    ...
    def _check_cancelled(
        self, task_id: str, cancel_token: CancellationToken | None
    ) -> None:
    ...
    def _drive(
        ...
        try:
            ...
            plan = self._plan(
                ...
                goal_text,
                clock,
                context=self._recall(task, ctx, goal_text, clock),
            )
            ...
            if cancel_token is not None and cancel_token.cancelled:
                return self._cancelled(
                    task,
                    ctx,
                    f"task {task.id} cancelled by operator",
                    clock,
                    total_started,
                )
            return self._fail(
                task, ctx, f"{type(exc).__name__}: {exc}", clock, total_started
            )
    ...
    def _understand(
        ...
        try:
            spec = request_structured(
                provider,
                ModelRequest(
                    prompt=f"UNDERSTAND: {goal_text}", system=_UNDERSTAND_SYSTEM
                ),
                TaskSpec,
            )
        ...
        if self._research is None:
            ...
            for item in result.evidence:
                ctx.observations.append(
                    f"evidence [{item.source_id[:8]}]: {item.claim}"
                )
    ...
    def _recover_if_needed(
        ...
        try:
            ...
            planner = (
                self._recovery_planner_factory()
                if self._recovery_planner_factory
                else None
            )
    ...
    def _complete(
        ...
        if current is not None and current.state is TaskState.CANCELLED:
            return self._cancelled(
                task, ctx, "cancelled by operator", clock, total_started
            )
    ...
    def _verify_one(
        self, task_id: str, step_id: str, results: list
    ) -> VerificationResult:
    ...
    def _worst(task_id: str, results: list[VerificationResult]) -> VerificationResult:
        ...
        return min(results, key=lambda r: order.get(r.status, 5))
