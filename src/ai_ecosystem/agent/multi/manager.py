...
class AgentManager:
    ...
    def run_task(
        ...
        self._emit(
            EventType.AGENT_STARTED, record.task_id, {"agent_id": record.owner_agent}
        )
        try:
...
class SupervisorResult:
    ...
    def __init__(
        self, task_ids: list[str], results: dict[str, ExecutionResult]
    ) -> None:
...
class Supervisor:
    ...
    def verify_results(
        ...
        for task_id in supervisor_result.task_ids:
            ...
            step_criteria = (criteria or {}).get(
                task_id, [{"strategy": "command_results"}]
            )
    ...
    def _default_decompose(
        goal: str, agents: list[AgentDefinition]
    ) -> list[SubtaskSpec]:
