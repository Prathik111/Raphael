...
class AgentRegistry:
    ...
    def register(self, definition: AgentDefinition) -> AgentDefinition:
        ...
        self._emit(EventType.AGENT_REGISTERED, "", {"agent_id": created.id, "role": created.role})
        return created
...
class AgentManager:
    ...
    def scoped_runner(self, agent_id: str) -> tuple[ToolRunner, AuthorizationManager]:
        ...
        authorizer = AuthorizationManager(
            self._tools, policy_engine=PolicyEngine(policy), context=RiskContext(agent_id=agent_id)
        )
    ...
    def run_task(self, record_id: str, cancel: CancellationToken | None = None) -> ExecutionResult:
        ...
        self._emit(EventType.AGENT_STARTED, record.task_id, {"agent_id": record.owner_agent})
        try:
        ...
        self._set_task_status(record, AgentStatus.COMPLETED if ok else AgentStatus.FAILED)
        record.result_summary = f"{len(result.succeeded)} succeeded, {len(result.failed)} failed"
    ...
    def run_all(
        self, record_ids: list[str], cancel: CancellationToken | None = None
...
class SupervisorResult:
    ...
    def __init__(self, task_ids: list[str], results: dict[str, ExecutionResult]) -> None:
    ...
    def succeeded(self) -> list[str]:
        ...
        return [tid for tid, result in self.results.items() if result.status.value == "COMPLETED"]
...
class Supervisor:
    ...
    def decompose(self, goal: str) -> list[SubtaskSpec]:
        ...
        agents = [
            d
            for d in self._manager._agents.list()
    ...
    def run_goal(
        self,
        goal: str,
        specs: list[SubtaskSpec] | None = None,
        ...
        records = [
            self._manager.submit(spec.agent_id, spec.goal, spec.plan, spec.arguments)
            for spec in chosen
        ]
    ...
    def verify_results(
        self, supervisor_result: SupervisorResult, criteria: dict[str, list] | None = None
    ) -> dict[str, Any]:
    ...
    @staticmethod
    def _default_decompose(goal: str, agents: list[AgentDefinition]) -> list[SubtaskSpec]:
