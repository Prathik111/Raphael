...
class Coordination:
    ...
    def __init__(
        self, correlation_id: str, results: dict[str, Any], verified: dict[str, bool]
    ) -> None:
...
class Coordinator:
    ...
    def fan_out(
        ...
        for spec in specs:
            record = self._manager.submit(spec.agent_id, spec.goal, spec.plan, spec.arguments)
            ...
            self._emit(EventType.AGENT_TASK_REQUESTED, record.task_id, {"agent_id": spec.agent_id})
        ...
        for record in records:
            ...
            self._messages.send(
                AgentMessage(
                    sender=record.owner_agent,
                    recipient=supervisor_id,
                    task_id=record.task_id,
                    message_type=MessageType.TASK_RESULT,
                    payload={"success": ok, "succeeded": execution.succeeded if execution else []},
                    correlation_id=correlation_id,
                )
            )
            self._emit(
            ...
            verified[record.owner_agent] = ok and self._verify_child(record.task_id, execution)
        if timed_out:
            self._emit(
                EventType.AGENT_TASK_RESULT,
                "",
                {"supervisor": supervisor_id, "detail": "fan-out deadline fired; partial results"},
            )
        return Coordination(correlation_id, results, verified)

    def chain(
        self, supervisor_id: str, steps: list[SubtaskSpec], correlation_id: str
    ) -> Coordination:
        ...
        for spec in steps:
            ...
            if previous_summary:
                self._messages.handoff(
                    supervisor_id, spec.agent_id, record.task_id, previous_summary, correlation_id
                )
            execution = self._manager.run_task(record.id)
            ok = execution.status is OverallStatus.COMPLETED
            results[spec.agent_id] = execution
            verified[spec.agent_id] = ok and self._verify_child(record.task_id, execution)
            previous_summary = f"{spec.agent_id}: {len(execution.succeeded)} steps succeeded"
            if not ok:
                break
        return Coordination(correlation_id, results, verified)
