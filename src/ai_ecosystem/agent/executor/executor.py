...
class ParallelExecutor:
    ...
    def __init__(self, runner: ToolRunner, registry: ToolRegistry, bus: EventBus | None = None, *, max_concurrency: int = 4,
                 failure_policy: FailurePolicy = FailurePolicy.FAIL_FAST, default_step_timeout_s: float = 60.0,
                 step_timeouts: dict[str, float] | None = None, poll_interval_s: float = 0.02) -> None:
    ...
    def execute_graph(self, task_id: str, graph: TaskGraph, *, arguments: dict[str, dict] | None = None, context: ExecutionContext | None = None,
        ...
        args = arguments or {}; token = cancel or CancellationToken(); started = time.monotonic(); self._emit(EventType.GRAPH_STARTED, task_id, {"steps": len(graph.nodes)}); self._persist(graph, context, contexts_repo)
        ...
        try:
            while not graph.done():
                ...
                self._collect_completed(graph, task_id, in_flight, context); self._enforce_deadlines(graph, task_id, in_flight)
                for node in graph.mark_unsatisfiable(): self._emit(EventType.STEP_SKIPPED, task_id, {"step_id": node.step.id, "error": node.error})
    ...
    def _run_node(self, task_id: str, node: GraphNode, arguments: dict, token: CancellationToken) -> tuple[bool, list[ToolResult], str]:
        try:
            ...
            for tool_name in node.step.tools:
                ...
                call = self._registry.build_call(task_id, tool_name, arguments); result = self._runner.run(call, cancel_token=token); results.append(result)
