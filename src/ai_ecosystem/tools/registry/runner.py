...
class GrantAllAuthorizer:
    def authorize(self, task_id: str, tool: Tool, call: ToolCall) -> Permission:
        return Permission(task_id=task_id, tool_call_id=call.id, decision=PermissionDecision.GRANTED,
                          reason="grant-all (tests only)", policy="grant-all")


class DenyAllAuthorizer:
    def authorize(self, task_id: str, tool: Tool, call: ToolCall) -> Permission:
        return Permission(task_id=task_id, tool_call_id=call.id, decision=PermissionDecision.DENIED,
                          reason="deny-all (tests only)", policy="deny-all")


class ToolRunner:
    """Runs validation -> authorization -> optional approval -> execution."""

    def __init__(self, registry: ToolRegistry, authorizer: Authorizer,
                 bus: EventBus | None = None, sandbox: Any = None,
    ...
    def _fail(self, call: ToolCall, event_type: EventType, error: str,
              tool: Tool | None = None, permission: Permission | None = None) -> ToolResult:
        error = _safe_error(error)
        call.status = ToolCallStatus.FAILED
        self._emit(event_type, call.task_id, {"tool": call.tool, "call_id": call.id, "error": error})
        self._audit(call, tool or call.tool, permission, False, error)
        return ToolResult(task_id=call.task_id, tool_call_id=call.id, success=False, error=error)
    ...
    def _deny(self, call: ToolCall, tool: Tool, reason: str) -> ToolResult:
        ...
        self._emit(EventType.PERMISSION_DENIED, call.task_id,
                   {"tool": call.tool, "call_id": call.id, "reason": reason})
        self._audit(call, tool, None, False, reason)
        return ToolResult(task_id=call.task_id, tool_call_id=call.id, success=False, error=reason)

    def _await_human(self, call: ToolCall, tool: Tool, permission: Permission,
                     cancel_token: Any = None) -> Permission:
        approval_id = getattr(permission, "approval_id", "") or ""
        self._emit(EventType.APPROVAL_REQUESTED, call.task_id,
                   {"tool": call.tool, "call_id": call.id, "approval_id": approval_id,
        ...
        if permission.decision is PermissionDecision.GRANTED:
            ...
            if not bound:
                permission.decision = PermissionDecision.DENIED
                permission.reason = "approval does not match this exact action (arguments changed after approval)"
        return permission
    ...
    def run(self, call: ToolCall, cancel_token: Any = None) -> ToolResult:
        ...
        if call.id in self._seen_ids:
            call.status = ToolCallStatus.FAILED
            return ToolResult(task_id=call.task_id, tool_call_id=call.id, success=False,
                              error=f"duplicate call id {call.id!r}: already executed; retries must use a new call")
        tool = self._registry.get(call.tool)
        if tool is None:
            return self._fail(call, EventType.TOOL_FAILED, f"unknown tool {call.tool!r}")
        self._emit(EventType.TOOL_REQUESTED, call.task_id,
                   {"tool": call.tool, "call_id": call.id, "arguments": _safe_payload(dict(call.arguments))})
        ...
        if permission.decision is not PermissionDecision.GRANTED:
            call.status = ToolCallStatus.DENIED
            self._emit(EventType.PERMISSION_DENIED, call.task_id,
                       {"tool": call.tool, "call_id": call.id, "reason": permission.reason})
        ...
        if cancel_token is not None and getattr(cancel_token, "cancelled", False):
            call.status = ToolCallStatus.DENIED
            return ToolResult(task_id=call.task_id, tool_call_id=call.id, success=False, error="cancelled before execution")
        self._emit(EventType.PERMISSION_GRANTED, call.task_id,
                   {"tool": call.tool, "call_id": call.id, "approval_id": getattr(permission, "approval_id", "") or ""})
        ...
        self._emit(EventType.TOOL_STARTED, call.task_id, {"tool": call.tool, "call_id": call.id})
        ...
        self._emit(EventType.TOOL_COMPLETED if result.success else EventType.TOOL_FAILED,
    ...
    def _run_sandboxed(self, call: ToolCall, tool: Tool, handler: Any, permission: Permission) -> ToolResult:
        ...
        if not isinstance(result, ToolResult):
            return self._fail(call, EventType.TOOL_FAILED, "sandbox returned a non-result", tool, permission)
        result.task_id = call.task_id
        result.tool_call_id = call.id
        call.status = ToolCallStatus.COMPLETED if result.success else ToolCallStatus.FAILED
        self._emit(EventType.TOOL_COMPLETED if result.success else EventType.TOOL_FAILED,
                   call.task_id, {"tool": call.tool, "call_id": call.id})
        self._audit(call, tool, permission, result.success, result.error or "", sandboxed=True)
        return result

    def _audit(self, call: ToolCall, tool: Any, permission: Any, success: bool,
               error: str, sandboxed: bool = False) -> None:
        ...
        try:
            ...
            self._auditor({"action": "tool.execute", "task_id": call.task_id,
                           ...
                           "success": success, "error": error, "sandboxed": sandboxed})
        except Exception:
            pass
