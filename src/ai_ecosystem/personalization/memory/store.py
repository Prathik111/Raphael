...
class MemoryStore:
    ...
    def store(self, candidate: MemoryCandidate) -> Memory:
        ...
        memory = Memory(
            ...
            provenance=str(
                candidate.metadata.get("provenance", candidate.source or "unknown")
            ),
    ...
    def retrieve(
        ...
        for memory in self._repo.list():
            ...
            scored.append(
                (round(score, 3), memory.created_at.isoformat(), memory.id, memory)
            )
    ...
    def purge_expired(self, now: datetime | None = None) -> list[str]:
        ...
        for memory in self._repo.list():
            expired_by_date = (
                memory.expires_at is not None and memory.expires_at <= moment
            )
            expired_by_retention = (
                memory.retention_days is not None
                and (moment - memory.created_at).total_seconds() / 86400.0
    ...
    def list_cloud_eligible(self) -> list[Memory]:
        return [
            ...
            and not (m.expires_at and m.expires_at <= utcnow())
        ]
