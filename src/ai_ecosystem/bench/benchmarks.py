...
def default_suite() -> BenchmarkRunner:
    ...
    from ai_ecosystem.agent.multi.messages import AgentMessage, MessageBus, MessageType
    ...
    from ai_ecosystem.scheduler import GlobalScheduler, SqliteScheduledJobRepository
    ...
    def world(context: dict) -> dict:
        if "world" not in context:
            ...
            agents.register(
                AgentDefinition(id="bench-agent", name="bench", allowed_tools=["work"])
            )
    ...
    runner.add(
        ...
        "runtime",
        50,
        lambda ctx, i: SqliteTaskRepository(world(ctx)["db"]).create(
            Task(title=f"t{i}")
        ),
    )
    runner.add(
    ...
    runner.add(
        ...
        lambda ctx, i: ParallelExecutor(
            world(ctx)["runner"], world(ctx)["registry"]
        ).execute(f"t{i}", plan()),
    )
    runner.add(
        "verify.command",
    ...
    runner.add(
        ...
        "memory",
        50,
        lambda ctx, i: SqliteMemoryRepository(world(ctx)["db"]).create(
            Memory(content=f"fact {i}")
        ),
    )
    runner.add(
        "memory.retrieve",
    ...
    runner.add(
        ...
        "ui",
        50,
        lambda ctx, i: WorkspaceManager(
            SqliteWorkspaceRepository(world(ctx)["db"])
        ).create(f"p{i}"),
    )
    runner.add(
        "viz.ingest",
