...
def measure(
    name: str, category: str, iterations: int, fn: Callable[[int], Any], extra: dict | None = None
) -> BenchmarkResult:
    ...
    def run(self) -> BenchmarkReport:
        ...
        for name, category, iterations, fn in self._cases:
            report.results.append(
                measure(name, category, iterations, lambda i: fn(self.context, i))
            )
        return report
...
def default_suite() -> BenchmarkRunner:
    ...
    def world(context: dict) -> dict:
        if "world" not in context:
            ...
            agents.register(AgentDefinition(id="bench-agent", name="bench", allowed_tools=["work"]))
            message_bus = MessageBus()
            message_bus.register("a")
            message_bus.register("b")
            skill_registry = SkillRegistry(SqliteSkillRepository(db))
            skill_registry.register(
                Skill(name="bench-skill", description="work helper", allowed_tools=["work"])
            )
            scheduler = GlobalScheduler(
                SqliteScheduledJobRepository(db),
                ComputeRouter({"local": ProviderCapabilities(name="local", local=True)}),
            )
            context["world"] = {
                "db": db,
                "runtime": runtime,
    ...
    runner.add(
        ...
        "runtime",
        50,
        lambda ctx, i: SqliteTaskRepository(world(ctx)["db"]).create(Task(title=f"t{i}")),
    )
    runner.add(
        "event.publish",
        "runtime",
        200,
        lambda ctx, i: world(ctx)["runtime"].bus.publish(
            Event(event_type=EventType.TASK_CREATED, task_id=f"t{i}")
        ),
    )
    runner.add(
        "plan.validate", "planning", 100, lambda ctx, i: PlanValidator({"work"}).validate(plan())
    )
    runner.add(
        "tool.execute",
        "execution",
        50,
        lambda ctx, i: world(ctx)["runner"].run(world(ctx)["registry"].build_call("t", "work", {})),
    )
    runner.add(
        "dag.execute",
        "execution",
        20,
        lambda ctx, i: ParallelExecutor(world(ctx)["runner"], world(ctx)["registry"]).execute(
            f"t{i}", plan()
        ),
    )
    runner.add(
        ...
        lambda ctx, i: Verifier().verify(
            "t", "s", [ToolResult(success=True, output="ok")], [{"strategy": "command_results"}]
        ),
    )
    runner.add(
        "memory.insert",
        "memory",
        50,
        lambda ctx, i: SqliteMemoryRepository(world(ctx)["db"]).create(Memory(content=f"fact {i}")),
    )
    runner.add(
        "memory.retrieve",
        "memory",
        20,
        lambda ctx, i: MemoryStore(SqliteMemoryRepository(world(ctx)["db"])).retrieve(
            MemoryScope.GLOBAL, query="fact"
        ),
    )
    runner.add(
        "skill.lookup", "skills", 100, lambda ctx, i: world(ctx)["skills"].lookup("bench-skill")
    )
    runner.add(
        "skill.discover", "skills", 50, lambda ctx, i: world(ctx)["skills"].select("work helper")
    )
    runner.add(
        "agent.delegate", "agents", 30, lambda ctx, i: world(ctx)["agents"].lookup("bench-agent")
    )
    runner.add(
        "message.delivery",
        "agents",
        100,
        lambda ctx, i: world(ctx)["messages"].send(
            AgentMessage(
                sender="a",
                recipient="b",
                task_id=f"t{i}",
    ...
    runner.add("scheduler.tick", "scheduling", 20, lambda ctx, i: world(ctx)["scheduler"].tick())
    runner.add(
        "provider.select",
        "cloud",
        100,
        lambda ctx, i: ComputeRouter(
            {
                ...
                "oci": ProviderCapabilities(name="oci"),
            }
        ).route(ComputeRequirements()),
    )
    runner.add(
        "sync.manifest",
        "cloud",
    ...
    runner.add(
        ...
        "ui",
        50,
        lambda ctx, i: WorkspaceManager(SqliteWorkspaceRepository(world(ctx)["db"])).create(
            f"p{i}"
        ),
    )
    runner.add(
        ...
        "ui",
        100,
        lambda ctx, i: EventAdapter().ingest(
            Event(event_type=EventType.TASK_CREATED, task_id=f"t{i}")
        ),
    )
    return runner
